...
public class CheatinElvesPart2 {
	...
	public static void main(String[] args) {
		...
		Scanner s = new Scanner(RPSInputs);
			...
			finalCount += (8 * valueCount[0]);
			finalCount += (4 * valueCount[1]);
			finalCount += (3 * valueCount[2]);
			finalCount += (9 * valueCount[3]);
			finalCount += (5 * valueCount[4]);
			finalCount += (1 * valueCount[5]);
			finalCount += (7 * valueCount[6]);
			finalCount += (6 * valueCount[7]);
			finalCount += (2 * valueCount[8]);
